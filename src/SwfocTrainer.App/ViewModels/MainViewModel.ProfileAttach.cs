...
public sealed partial class MainViewModel
{
    ...
    private static string BuildAttachProcessHintSegment(ProcessMetadata process)
    {
        ...
        var via = ReadProcessMetadata(process, "detectedVia", MainViewModelDefaults.UnknownValue);
        var launch = launchContext?.LaunchKind.ToString() ?? "n/a";
        var recommended = launchContext?.Recommendation.ProfileId ?? string.Empty;
        var reason = launchContext?.Recommendation.ReasonCode ?? MainViewModelDefaults.UnknownValue;
    ...
    {
        return process.CommandLine?.Contains(workshopId, StringComparison.OrdinalIgnoreCase) is true;
    }
    ...
    private static string BuildRecommendationSegment(ProcessMetadata process)
    {
        ...
        var reason = recommendation?.ReasonCode ?? MainViewModelDefaults.UnknownValue;
