...
PluginResult GlobalTogglePlugin::execute(const PluginRequest& request) {
    ...
    if (const auto encoded = static_cast<std::uint8_t>(nextValue ? 1 : 0);
        !process_mutation::TryWriteValue<std::uint8_t>(request.processId(), targetAddress, encoded, writeError)) {
