...
public sealed class LaunchContextScriptSmokeTests
{
    ...
    {
        var actualByCaseName = TryCollectScriptResultsByCaseName();
        if (actualByCaseName is null)
        {
            return;
        }

        AssertExpectedRecommendations(actualByCaseName, BuildExpectedRecommendations());
    }

    private static IReadOnlyDictionary<string, JsonObject>? TryCollectScriptResultsByCaseName()
    {
        ...
        using var process = TryStartScriptProcess(root, scriptPath, fixturePath, python.Value);
        if (process is null)
        {
            return null;
        }
    ...
    private static Process? TryStartScriptProcess(
        string root,
        string scriptPath,
        string fixturePath,
    ...
    private static IReadOnlyDictionary<string, (string ProfileId, string ReasonCode)> BuildExpectedRecommendations()
        => new Dictionary<string, (string ProfileId, string ReasonCode)>(StringComparer.OrdinalIgnoreCase)
        {
    ...
    private static void AssertExpectedRecommendations(
        IReadOnlyDictionary<string, JsonObject> actualByCaseName,
        IReadOnlyDictionary<string, (string ProfileId, string ReasonCode)> expectedByCaseName)
    {
        foreach (var (caseName, expectedRecommendation) in expectedByCaseName)
        {
            actualByCaseName.Should().ContainKey(caseName);
            var recommendation = actualByCaseName[caseName]["profileRecommendation"]!;
