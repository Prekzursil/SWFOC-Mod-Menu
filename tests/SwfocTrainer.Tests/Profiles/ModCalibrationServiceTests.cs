...
public sealed class ModCalibrationServiceTests
{
    ...
    public async Task BuildCompatibilityReportAsync_ShouldBlockPromotion_WhenCriticalSymbolsUnresolved()
    {
        ...
        var session = new AttachSession(
            ...
            Process: new ProcessMetadata(1234, "StarWarsG.exe", @"C:\Games\StarWarsG.exe", "STEAMMOD=555000111", ExeTarget.Swfoc, RuntimeMode.Galactic,
                new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                {
                    ["dependencyValidation"] = "Pass"
                }),
            Build: new ProfileBuild(profile.Id, "test", @"C:\Games\StarWarsG.exe", ExeTarget.Swfoc, "STEAMMOD=555000111", 1234),
            Symbols: symbols,
