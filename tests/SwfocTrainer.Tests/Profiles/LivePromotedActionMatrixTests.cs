            ...
            foreach (var profileId in TargetProfiles)
            {
                ...
                try
                {
                    ...
                    foreach (var actionSpec in PromotedActions)
                    {
                        ...
                        var hasFallbackMarker = HasHybridFallbackMarker(
                            result,
                            backendRoute,
                            routeReasonCode,
                            result.Diagnostics);
                        ...
                        result.Succeeded.Should().BeTrue(
                            $"promoted action '{actionSpec.ActionId}' should execute successfully for profile '{profileId}'. message={result.Message}");
                        backendRoute.Should().Be(
                            ExecutionBackendKind.Extender.ToString(),
                            because: $"promoted action '{actionSpec.ActionId}' should route via extender backend for profile '{profileId}'.");
                        routeReasonCode.Should().Be(
                            RuntimeReasonCode.CAPABILITY_PROBE_PASS.ToString(),
                            because: $"promoted action '{actionSpec.ActionId}' should pass route capability gate for profile '{profileId}'.");
                        capabilityProbeReasonCode.Should().Be(
                            RuntimeReasonCode.CAPABILITY_PROBE_PASS.ToString(),
                            because: $"promoted action '{actionSpec.ActionId}' should report capability probe pass for profile '{profileId}'.");
                        hybridExecution.HasValue.Should().BeTrue(
                            because: $"promoted action '{actionSpec.ActionId}' should emit hybrid execution diagnostics for profile '{profileId}'.");
                        hybridExecution.Should().BeFalse(
                            because: $"promoted action '{actionSpec.ActionId}' should execute as native-authoritative extender flow for profile '{profileId}'.");
                        hasFallbackMarker.Should().BeFalse(
                            because: $"promoted action '{actionSpec.ActionId}' should not include fallback markers for profile '{profileId}'.");
                    }
                }
                finally
                {
