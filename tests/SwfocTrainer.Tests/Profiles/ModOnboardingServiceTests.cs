...
public sealed class ModOnboardingServiceTests
{
        ...
        return new ModOnboardingSeedBatchRequest(
            ...
            Seeds: new[]
            {
                new GeneratedProfileSeed(
                    ...
                    RequiredCapabilities: new[] { "set_credits", "set_unit_cap" },
                    AnchorHints: new[] { "workshop:777888999", "title:metadata" },
                    RiskLevel: "high",
                    ParentDependencies: new[] { "1397421866" },
                    LaunchHints: new[] { "workshop", "launch_swfoc" },
                    Title: "Metadata Seed",
    ...
    private static void AssertMetadataSeedDraft(TrainerProfile? draft)
    {
        ...
        draft.Metadata!["riskLevel"].Should().Be("high");
        draft.Metadata!["parentDependencies"].Should().Contain("1397421866");
        draft.Metadata!["launchHints"].Should().Contain("launch_swfoc");
        draft.Metadata!["anchorHints"].Should().Contain("workshop:777888999");
        draft.Metadata!["requiredCapabilities"].Should().Contain("set_unit_cap");
        draft.FeatureFlags["allow_fog_patch_fallback"].Should().BeFalse();
        draft.FeatureFlags["allow_unit_cap_patch_fallback"].Should().BeFalse();
        draft.FeatureFlags["requires_calibration_before_mutation"].Should().BeTrue();
    }
